...
void server_add_client(server* srv){
    ...
    new_client->socket = accept(srv->socket, (struct sockaddr*) &(new_client->addr), &(new_client->addr_len));
...
void server_run(server* srv){
    ...
    int i, ready;
    while(srv->active){
        fds = (struct pollfd *) realloc(fds, (srv->n_clients + 2) * sizeof(struct pollfd));
        ...
        client* c;
        for (i = 2, c = srv->clients; (i < srv->n_clients + 2) && (c != NULL); i++, c = c->next) {
            fds[i].fd = c->socket;
            fds[i].events = POLLIN;
        }
        ...
        if (ready > 0){
            ...
            client* next;
            for (i = 2, c = srv->clients; (i < srv->n_clients + 2) && (c != NULL); i++, c = next) {
                next = c->next;
                if (fds[i].revents == POLLIN) {
                    ...
                    if (c->state == IDLE) {
                        ...
                        if (nbytes > 0) {
                            ...
                            get_ip_str((struct sockaddr *) &(c->addr), addr, INET6_ADDRSTRLEN);
