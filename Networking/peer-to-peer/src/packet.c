...
packet *packet_decode_hdr(const unsigned char *buffer, size_t buf_len) {
    ...
    if (!(p->flags & PKT_FLAG_CTRL)) {
        ...
        p->node_ip = (buffer[5] << 24u) | (buffer[6] << 16u); // Upper part of IP only -> the rest is in the 'body'
    }

    return p;
}

packet *packet_decode_body(packet *p, const unsigned char *buffer, size_t buf_len) {
    if (p->flags & PKT_FLAG_CTRL) {
        p->node_ip |= (buffer[0] << 8u) | (buffer[1] << 0u); // Lower part of IP only -> the rest is in the 'header'
    ...
    if (buf_len < pkt_size) {
        fprintf(stderr, "Buffer shorter than expected from header! (Expected: %zu Got: %zu)\n", pkt_size, buf_len);
        packet_free(p);
        return NULL;
    }
    ...
    //NOTE: We should check for ENOMEM because values can be up to 2^32 Bytes long...
    if (p->value_len > 0){
